			...
			.ready(
					function() {
								...
								.click(
										function add() {
											...
											registerRequset
													.open(
															"Post",
															"./ReserveCommander?seat="
																	+ encodeURIComponent(document
																			.getElementById("seat").value)
																	+ "&mvarea="
																	+ encodeURIComponent(document
																			.getElementById("mvarea").value)
																	+ "&mvname="
																	+ encodeURIComponent(document
																			.getElementById("mvname").value)
																	+ "&mvtime="
																	+ encodeURIComponent(document
																			.getElementById("mvtime").value)
																	+ "&mvdate="
																	+ encodeURIComponent(document
																			.getElementById("mvdate").value)
																	+ "&mvprice="
																	+ encodeURIComponent(document
																			.getElementById("mvprice").value),
															true);
						...
						function registerProcess() {
							if (registerRequset.readyState == 4
									&& registerRequset.status == 200) {
								var result = registerRequset.responseText;
								alert(registerRequset.responseText);
								if (result != 1) {
									alert("등록 실패");
								} else {
									var seatValue = document
											.getElementById("seat");
									seatValue.value = "";
